...
public class Bank
{
	...
	*/
	 ...
	 */
	public int findIndexByName(String name) //search
	{
		for(int i = 0 ; i < nClients ; i++)
		{
			//if (this.names[i] == name) //do not do it for String
	...
	{
		//if (nClients < balance.length)
		if (nClients < names.length) //check for space
		{
			int index = findIndexByName(name);  //check if it is already in the bank
	...
	{
		int index = findIndexByName(name); //search for the client
	...
	{
		int index = findIndexByName(name); //search for the name

		if (index == -1) //not found
			return false;
		else
		{
			double t = balances[index]; //store the balance before changing
	...
	{ 
		...
		else
		{
			...
			if (balances[index] >= amount) //if the balance is enough
			{
	...
	{ 
		//fname : From name
		//tname : To name
		int index1 = findIndexByName(fName); //search for from name

		if (index1 == -1) //from name not in the bank, get out
			return false;
		else
		{
			int index2 = findIndexByName(tName); //search for to name
			
			if (index2 == -1) //to name not in the bank, get out
				return false;
			else
			{
	...
	public boolean transfer2(String fName, String tName, double amount)
	{ 
		//fname : From name
		//tname : To name
		int index1 = findIndexByName(fName); //search for from name
